...
arguments:
    - shellQuote: false
      valueFrom: >
        ...
        #cp \${FILTERED_FILE} "$(runtime.outdir)/$(inputs.out_file_name)"

        awk 'BEGIN{FS="\t";OFS="\t"}{ if ( $11 >= 4) { $13 = ($12 / $11); print $0 } }' \${FILTERED_FILE} > "$(runtime.outdir)/$(inputs.out_file_name)"

        #awk 'BEGIN{FS="\t";OFS="\t"}{ if ( $11 >= 4) { $13 = ($12 / $11); print $0 } }' \${FILTERED_FILE} | awk '$2!=N{print $0}' > "$(runtime.outdir)/$(inputs.out_file_name)"
inputs:
  in_file:
    type: File
  out_file_name:
